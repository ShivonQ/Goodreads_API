...
class goodreads_client():
    def __init__(self):
        self.secret_key = 'cdaBdgcnNZnxU2rwQOUREQ'       #secret_key
    ...
    def search(self, parameter):
        ...
        base_url = 'https://www.goodreads.com/search/index.xml?q='+parameter+'&key='+self.secret_key
    ...
    def all_books_by_author(self, auth_id):
        base_url = 'https://www.goodreads.com/series/list/'+auth_id+'.xml?key='+self.secret_key
        response = requests.post(base_url)
        dict_form = ET.fromstring(response.text)
        results = parse_best_books(dict_form)
        return results
    ...
    def author_by_name(self, author_name):
        base_url = 'https://www.goodreads.com/api/author_url/' + author_name + '?key=' + self.secret_key
...
def parse_best_books(dict_form):
    ...
    for best_book in dict_form.iter('best_book'):
        ...
        #     book['image'] = image_url.text  #from secret_key import secret_key
